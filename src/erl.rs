...
pub fn handle(fname: &str, funs: &Vec<Fun>, sheet_map: &HashMap<String, Sheet>) {
    ...
    buffer.write_fmt(format_args!("-module({}).\n-export([\n",f_list.last().unwrap().to_string())).unwrap();

    let erl: Vec<&Fun> = funs.iter().filter(|f| f.lang.contains("erl")).collect();
    match erl.len() {
        0 => (),
        1 => buffer.write_fmt(format_args!("\t{}/1\n",erl[0].fun)).unwrap(),
        _ => {
            buffer.write_fmt(format_args!("\t{}/1\n",erl[0].fun)).unwrap();
            for i in 1..erl.len() {
                buffer.write_fmt(format_args!("\t,{}/1\n",erl[i].fun)).unwrap();
            }
        }
    ...
    for data in &s.data {
        ...
        if args.is_some() {
            ...
            match f.symbol.as_str() {
                "=" => buffer.write_fmt(format_args!("{}({}) -> {};\n", f.fun, args.unwrap(), values)).unwrap(),
                _ => buffer.write_fmt(format_args!("{}(T) when T {} {} -> {};\n", f.fun, f.symbol, args.unwrap(), values)).unwrap(),
            }
